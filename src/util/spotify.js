const clientId = 'c768ff6f5e5b4681bce7c009431a9c9e';
const redirectUri = 'http://jamproj.surge.sh'
let accessToken;
...
const spotify = {
    ...
    savePlaylist(name,trackrUris){
        if(!name||!trackrUris.length){
            return;
        }
        ...
        .then(jsonResponse => {
            ...
            return fetch(`https://api.spotify.com/v1/me/v1/users/${userId}/playlists/${playlistId}/tracks`,{
                headers: headers,
                method: 'POST',
                body: JSON.stringify({uris: trackrUris})
            })
        })
        })
    }
